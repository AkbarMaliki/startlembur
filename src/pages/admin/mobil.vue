  ...
  <div class="font-times">
    ...
    >
     <div class="card">
      ...
      <!-- FORM --> 
          <tr>
              <td class='text-uppercase text-sm text-capitalize'>id</td>
              <td>&nbsp;:&nbsp;</td>
          ...
          <tr>
              <td class='text-uppercase text-sm text-capitalize'>nama_mobil</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="nama_mobil_"
                    id="nama_mobil"
                    name="nama_mobil"
                    class="form-control form-control-sm"
                    v-model="vdata.nama_mobil"
          ...
          <tr>
              <td class='text-uppercase text-sm text-capitalize'>plate</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
          ...
          <tr>
              <td class='text-uppercase text-sm text-capitalize'>driver</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <v-select :options="drivers" label="nama" v-model="vdata.driver"></v-select>
              </td>
            </tr>
             
          <tr>
              <td class='text-uppercase text-sm text-capitalize'>unit</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form ">
                   <div class="tips">
                    <input type="unit" id="unit" name="unit ..." class="form-control form-control-sm" placeholder="unit" v-model="vdata.unit" >
                      <span class="tipstextB">Contoh : BCSU</span>
                  </div>
                </div>
              </td>
            </tr>
                  <tr>
              <td class='text-uppercase text-sm text-capitalize'>Keterangan</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
              <div class="sm-form">
              <textarea
        ...
        <img :src="vdata.gambar" class="kinoLightBox img-fluid " style="height:100px;">
      ...
      <!-- SUBMIT -->
        ...
        <div class="p-3">
            ...
            <div class="p-3" v-if="imageData!=null">
                <img class="preview" :src="picture">
                <br>
              <button @click="onUpload" class="btn btn-sm btn-success"><span class="typcn typcn-upload"></span> Upload</button>
            </div>
     <hr class="style13" />
     <div class="row">
...
export default {
  ...
  data() {
    return {
      ...
      datanya: [],
      drivers: [],
      order:['id','nama_mobil','plate','unit','gambar','driver','keterangan'],
      vdata: {
  ...
  methods: {
    ...
    insert(){
          ...
          db.collection(`mobil`).doc().set(this.vdata).then(res=>{
              console.log('berhasil')
              alert('simpan data berhasil!')
          })
      },update(){
          db.collection(`mobil`).doc(this.pilih.id).set(this.vdata,{merge:true}).then(res=>{
              alert('update data berhasil!')
          })
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(`mobil`).doc(this.pilih.id).delete().then(res=>{
       ...
       refreshData() {
         let that = this
         db.collection(`mobil`).onSnapshot(snap=>{
             let arr = []
             snap.forEach(e=>{
    ...
    checkedAll() {
     ...
     susun(datas){
      let that=this
      let data = datas;
      let order = ['id','nama_mobil','plate','gambar','driver','unit']
      let hasil=[]
      let obj={}
  ...
  mounted() {
    let that = this;
    db.collection('unit').get().then(res=>{
      let data = res.docs.map(e=>e.data())
      this.units = data
    })
    db.collection('users').where('unit.nama_unit','==','DRIVER').get().then(res=>{
      let data = res.docs.map(e=>e.data())
      this.drivers = data
    })
    this.refreshData();
