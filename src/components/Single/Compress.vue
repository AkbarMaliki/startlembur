<template>
    <div>
        <div class="sm-form ">
             <div class="tips">
                    <label for="persen">Your persen</label>
                    <input type="number" id="persen" name="persen" min="1" max="100" class="form-control form-control-sm" placeholder="persen" v-model="persen" >
                <span class="tipstextB" >100 = original size</span>
            </div>
        </div>
        data akan dikompres menjadi {{persen}}
        <hr class="style2">
        <div class="uploadContainer hoverable text-center flex flex-wrap flex-row" style="padding:10px;">
            <div class="imgInputContainer " @click="upload" style="cursor:pointer;margin-left:0px;">
                <span  v-for="(item, index) in imgList" :key="index" style="position:relative;">
                     <div class="tips">
                    <a :href="item" download="new.jpg"><img :id="'imgKu'+index" class="imgInput myImage"   :src="item" style="width:50%;z-index:100;" v-show="imgList.length>0" ></a>
                        <span class="tipstextB" style="position:absolute;top:0;">Click to Download</span>
                    </div>
                </span>
            </div>
            <div style="position:static;" class="text-center cursor-pointer"
             >
                <label for="file" v-if="imgList.length<1" style="font-size:12px;color:black;position:absolute;margin-left:13px;margin-top:23px;">Drag/Drop file</label>
                <!-- <label for="file" style="position:absolute;margin-left:50px;margin-top:30px;z-index:-1;color:#ffffff07;">test</label> -->
                <input class="cursor-pointer" id="file" type="file" @input.prevent="ambil($event)" :class="{'d-none':imgList.length>0}" multiple ref="uploader"  />
            </div>
