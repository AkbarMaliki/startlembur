<template>
  <div>
    ...
    <br />
    <div class="p-3">
       <p class="text-center font-times text-xl">Peminjaman Barang</p>
      <br>
      ...
      <div v-if="edit">
        <br />
        <form action @submit.prevent="pinjambarang" class="animate__animated animate__fadeIn">
          <div class="sm-form">
          ...
          <div class="sm-form">
           ...
           <div class="sm-form">
            <label for="waktu_ambil">Waktu ambil</label>
            <input
              type="date"
              id="waktu_ambil"
              name="waktu_ambil"
              class="form-control form-control-sm"
              placeholder="waktu_ambil"
              v-model="vdata.waktu_ambil"
            />
          </div>
            <div class="sm-form">
            ...
            <input
              ...
              name="jam_ambil"
              class="form-control form-control-sm"
              placeholder="jam_ambil"
              v-model="vdata.jam_ambil"
            />
          </div>
          <br />
      ...
      <button type="button" @click="hapus(item.id)" class="btn btn-sm btn-danger  float-right"  v-show="item.uid==$store.state.users.uid">x</button>
        ...
        <p class="font-bold">
          ...
          <span class="text-capitalize pl-2">{{format(item.waktu_ambil)}}</span>
        </p>
        <p class="font-bold">
          <span>Jam ambil :</span>
          <span class="text-capitalize pl-2">{{item.jam_ambil}}</span>
        </p>
      </div>
      <br>
...
export default {
  ...
  methods: {
    ...
    pinjambarang() {
      ...
      db.collection("peminjaman_barang")
        .doc()
    ...
    hapus(id){
        if(confirm('Yakin menghapus barang ? ')){
            db.collection('peminjaman_barang').doc(id).delete().then(res=>{
  ...
  computed:{
    ...
    //   }
      ...
      if (data[0] != undefined) {
       ...
       data = data.filter((v, i, a) => {
        let hasil = false;
        console.log(v.waktu_ambil==that.tanggal)
          if(v['waktu_ambil']==this.tanggal){
            return v
  ...
  mounted() {
    ...
    this.vdata.waktu_ambil = this.format2(new Date())
    this.$forceUpdate()
    db.collection("peminjaman_barang").onSnapshot((res) => {
