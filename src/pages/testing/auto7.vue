    ...
    getData() {
      ...
      if(this.vdata.table.includes('/',0)){
        ...
        // this.vdata.table=leng
         db.collectionGroup(`${leng}`).get().then(res=>{
            let arr=[]
            res.forEach(e=>{
                arr.push({id:e.id,...e.data()})
            })
            this.datanya = Object.keys(arr[0]);
             let obj=[];
          ...
          db.collectionGroup(`${this.vdata.table}`).get().then(res=>{
            let arr=[]
            res.forEach(e=>{
                arr.push({id:e.id,...e.data()})
            })
  ...
  methods: {
    ...
    `;
      let methods = `insert(){
          ...
          db.collection(\`${this.vdata.table}\`).doc(this.pilih.id).set(this.vdata,{merge:true}).then(res=>{
              alert('update data berhasil!')
          })
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(\`${this.vdata.table}\`).doc(this.pilih.id).delete().then(res=>{
              alert('delete data berhasil!')
              })
          }
      },
       refreshData() {
         let that = this
         db.collection(\`${this.vdata.table}\`).onSnapshot(snap=>{
             let arr = []
             snap.forEach(e=>{
                 arr.push({id:e.id,...e.data()})
             })
