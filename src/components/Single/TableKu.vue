              ...
              <div class="row">
                <div class="col-sm-12">
                  <button
                    ...
                    style="position:absolute;right:15px;top:55px;"
                    @click="modal=false"
                  >X</button>
                  <div
                    class="text-center"
                    style="background:lightgray;margin-top:10px;border:1px solid white;box-shadow:1px 1px 1px black;"
                  >
                  ...
                  <form action @submit.prevent="Edit">
                    <div class="sm-form" v-for="(item,key, index) in dataTerpilih" :key="index">
                      <section>
                        <span
                          style="font-family:times new roman;margin-left:10px;text-transform:capitalize;"
                        >
                          {{key}}
                          <strong>:</strong>
                        </span>
                ...
                <div class="col-sm-12">
                  ...
                  <form action @submit.prevent="Save">
                    <div class="sm-form" v-for="(item, index) in dataSaveFields" :key="index">
                      <section v-if="true">
                        <!-- cek jika type nya ingin berbeda -->
                        <label :for="item">{{item}}</label>
                        <input
                          :type="item=='password'?'password':item=='email'?'email':'text'"
                          class="form-control form-control-sm"
                          placeholder=" . . . "
                          @keyup="saveData($event,item)"
                        >
                      </section>
                    </div>
                    <div class="sm-form">
      ...
      <section id="filterModal" v-if="filterModal" @mouseover="closeModal">
        <div class="modalDialog">
          <div class="delCon text-center">
            ...
            >X</button>
            <hr>
            <h4 style="color:red;">Cari data antara :</h4>
            <hr>
            <div class="sm-form">
              <input
                type="text"
      ...
      <div class="container-fluid">
        <div class="row">
          <div class="col-sm-4 col-12 fixZ flexRow">
            <JsonToExcel
              class="btn btn-sm btn-outline-success waves-effect no-print"
              :data="filData"
            />
            <button
              type="button"
              @click="printIni"
              class="btn btn-sm btn-outline-danger waves-effect no-print"
            >
              <i class="fas fa-print"></i>
              <span style="font-size:10px;">Print</span>
            </button>
            <button
              type="button"
            ...
            >
              ...
              <span style="font-size:10px;">Filter</span>
            </button>
            <section v-if="mode==true">
              <button
                type="button"
                @click="saveD"
                class="btn btn-sm btn-outline-secondary waves-effect no-print"
              >
                +
                <span style="font-size:10px;">Tambah</span>
              </button>
            </section>
          ...
          <div class="col-sm-6 searchField">
            <b-form-input
              type="text"
              style="width:300px;"
              class="zIndex form-control form-control-sm"
              placeholder="Pencarian "
              id="searchField"
              v-model="searchFields1"
            ></b-form-input>
          </div>
    ...
    <section id="mainTable">
      <div class="table-responsive-sm">
          ...
          <thead class="mdb-color bg-dark text-white table-print" id="tableKu1">
            <tr class="text-black table-print">
              ...
              <th
                ...
                class="taufik table-print"
                @click="sort($event,key,$refs[key])"
                v-for="(value,key, index) in filData[0]"
                :key="index"
              >
                {{key.toUpperCase()}}
                <span :ref="key" aria-hidden="true" style="display:none">
                  <i class="fas fa-sort no-print"></i>
                </span>
              </th>
            </tr>
          </thead>
          ...
          <tbody id="tbody">
            ...
            <tr class="table-print deletHvr" v-for="(item, index) in filData" :key="index">
              ...
              <td
                class="table-print"
                style="cursor:pointer;"
                @click.stop="pilihan($event,item,index)"
                v-for="(item2,key, index2) in item"
                :key="index2"
              >
                ...
                {{item2}}
                <button
                  class="btn btn-sm btn-danger deleteBtn animated fadeInLeft"
                  v-if="key==lastKey"
                  @click="Delete(index)"
                >X</button>
              </td>
        ...
        <section class="no-print text-center" id="pageButton">
          <button
            @click="nextPagination(-1)"
            type="button"
            class="btn btn-md btn-dark fadeING"
            :disabled="pagi1==0"
          >
          ...
          <button
            ...
            type="button"
            class="btn btn-md btn-dark fadeING"
            :disabled="pagi2>=dataTable.length"
          >
...
export default {
  ...
  computed: {
    ...
    filData() {
      ...
      let hasilData = this.dataTable.filter((v, i, a) => {
        ...
        keys.filter(key => {
          if (typeof v[key] == "string") {
            if (
              v[key].toLowerCase().indexOf(this.searchFields1.toLowerCase()) !=
              -1
            ) {
              hasil = true;
            }
      ...
      if (this.sortnya) {
        if (this.sortr) {
          hasilData = hasilData.sort(function(a, b) {
            return a[that.sortnya] > b[that.sortnya]
              ? 1
              : b[that.sortnya] > a[that.sortnya]
              ? -1
              : 0;
          });
        } else {
