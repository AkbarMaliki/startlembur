    ...
    <!-- FORM -->
      ...
      <div class="row">
        <div class="offset-sm-2 col-sm-8">
            ...
            <div class="col-4">
                <div class="p-2">
                    <button type="button"  class="btn btn-sm btn-style9 " @click="$router.push('/admin/user/add')">ADD USER <span class="typcn typcn-user-add"></span></button>
                </div>
            </div>
            <div class="col-4">
                <div class="p-2">
                    <button type="button"  class="btn btn-sm btn-style9 " @click="editnya()" style="background:lightblue;">EDIT USER <span class="typcn typcn-user-add"></span></button>
                </div>
            </div>
            <div class="col-4">
                 <div class="p-2">
                    <button type="button"  class="btn btn-sm btn-style9 " @click="hapus()" style="background:red;">DELETE <span class="typcn typcn-arrow-repeat"></span></button>
                </div>
            </div>
            <div class="col-12 pl-4 pr-4 font-bold text-xl">
                {{vdata.nama}} {{vdata.nip}}
            </div>
            </div>
    <!-- <form
      action
    ...
    >
     <div class="card">
        ...
        <div class="card-body">
          ...
          <tr>
              <td class='text-uppercase text-sm'>uid_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
          ...
          <tr>
              <td class='text-uppercase text-sm'>changepassword_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                  <select class='form-control' v-model="vdata.changepassword">
          ...
          <tr>
              <td class='text-uppercase text-sm'>pertama_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                  <select class='form-control' v-model="vdata.pertama">
                <option value="true">off</option>
                <option value="false">on</option>
                </select>
              </td>
          ...
          <tr>
              <td class='text-uppercase text-sm'>auth_</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <select class='form-control' v-model="vdata.auth">
                <option>admin</option>
                <option>user</option>
                </select>
              </td>
          ...
          <tr>
              <td class='text-uppercase text-sm'>bagian</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                  <select class='form-control' v-model="vdata.bagian">
                <option>-</option>
                <option>deputy</option>
                <option>unit_head</option>
                </select>
              </td>
            </tr>
      <br />
...
export default {
  ...
  data() {
    return {
      ...
      // valdatable
      datanya: [],
      order:['id','nip','nama','bagian','uid','changepassword','pertama','auth'],
      vdata: {
      ...
      err: false,
      checkedItem:[],
      less: [,'_id','id','uid','changepassword','pertama','auth'],
      // modal
      modal: false,
      modalAction: "preview",
      // pagination
  ...
  methods: {
      editnya(){
          if(!this.vdata.id){
              alert("Silahkan pilih salah satu ID atau user!")
          }else{
              this.$router.push(`/admin/user/edit?id=${this.vdata.id}`)
          }
      },
       hapus(){
          if(!this.vdata.id){
              ...
              if(confirm("Apakah yakin user dihapus ?")){
                db.collection('users').doc(this.vdata.id).delete().then(res=>{
    ...
    insert(){
          ...
          db.collection(`users`).doc().set(this.vdata).then(res=>{
              console.log('berhasil')
              alert('simpan data berhasil!')
          })
      },update(){
          db.collection(`users`).doc(this.pilih.id).set(this.vdata,{merge:true}).then(res=>{
              alert('update data berhasil!')
          })
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(`users`).doc(this.pilih.id).delete().then(res=>{
    ...
    checkedAll() {
     ...
     susun(datas){
      let that=this
      let data = datas;
      let order = ['id','nip','nama','uid','changepassword','pertama','auth','head']
      let hasil=[]
      let obj={}
