        ...
        <div class="container-fluid" style="padding:10px 40px 40px 20px;">
            <div class="row">
                <div class="col-sm-3">
                    <p class="p-2" style="color:red;font-size:12px;font-family:times new roman;">
                        Note: Untuk saat ini ulasan tidak bisa di ubah kalau sudah di input
                    </p>
            ...
            <div class="row isiTable " v-for="(item, index) in keranjang" :key="index" style="box-shadow:1px 2px 4px gray;" >
                ...
                <div class="col-sm-2 actionPage ">
                    <p v-if="item.idPembeli==$auth.user._id">BARANG DIBELI</p>
                    <p v-else>BARANG DIJUAL</p>
                    <button type="button" class="btn btn-sm btn-success  btn-block" style="font-size:13px;"
                    @click="$nuxt.$emit('modal',true);$nuxt.$emit('modalId','InfoPembayaran');$route.query.idTransaksi=item._id;$route.query.bayar=true"
                    >Info Barang </button>
                     <button type="button" class="btn btn-sm btn-warning  btn-block" style="font-size:13px;"
                    @click="$nuxt.$emit('modal',true);$nuxt.$emit('modalId','FormUlasan');$nuxt.$emit('transaksiComponent','Dikeranjang');$route.query.barangId=item.barangId;$route.query.idTransaksi=item._id;" 
                    v-if="item.idPembeli==$auth.user._id && item.star1==0 && item.star2==0 && item.star3==0 && item.star4==0 && item.star5==0"
                    >Ulasan</button>
                    <button type="button" class="btn btn-sm btn-warning  btn-block" style="font-size:13px;"
                    @click="$nuxt.$emit('modal',true);$nuxt.$emit('modalId','FormUlasan');$route.query.barangId=item.barangId;$route.query.idTransaksi=item._id;" 
                    v-else disabled
                    >Ulasan</button>
                </div>
            </div>
            ...
            if(data[0]!=undefined){
                if(this.$auth.user.email=='admin@gmail.com'){
                    ...
                    data=data.filter(e=>{
                        ...
                        console.log('user');
                        data=data.filter(e=>{
                            return e.status==5  && (e.idPembeli==this.$auth.user._id || e.idPenjual==this.$auth.user._id)
                        });
                }
                console.log('tes');
                if(data.length>0){
