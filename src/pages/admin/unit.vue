    ...
    >
     <div class="card">
      ...
      <!-- FORM --> 
          ...
          <tr>
              <td class='text-uppercase text-sm'>Unit</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="nama_unit_"
                    id="nama_unit"
                    name="nama_unit"
                    class="form-control form-control-sm"
                    v-model="vdata.nama_unit"
               ...
               <tr>
              <td class='text-uppercase text-sm'>bawahan dari</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
                <div class="sm-form">
                  <input
                    type="text"
                    placeholder="Bagian dari"
                    id="bagian_dari"
                    name="bagian_dari"
                    class="form-control form-control-sm"
                    v-model="vdata.bagian"
          ...
          <tr>
              <td class='text-uppercase text-sm'>Head Uid</td>
              <td>&nbsp;:&nbsp;</td>
              <td style="width:90%;padding-bottom:5px;">
...
export default {
  ...
  data() {
    return {
      ...
      // valdatable
      datanya: [],
      order:['id','nama_unit','bagian','head_uid','head_nama'],
      vdata: {
  ...
  methods: {
    unitnama(obj){
      ...
      let data = JSON.parse(obj.target.value)
      this.vdata.head_uid=data.id
      this.vdata.head_nama=data.nama
      this.$forceUpdate()
    },
    insert(){
          delete this.vdata.id;
          db.collection(`unit`).doc().set(this.vdata).then(res=>{
              console.log('berhasil')
              alert('simpan data berhasil!')
          })
      },update(){
          db.collection(`unit`).doc(this.pilih.id).set(this.vdata,{merge:true}).then(res=>{
              alert('update data berhasil!')
          })
      },remove(){
          if(confirm('Apakah yakin menghapus data?')){
              db.collection(`unit`).doc(this.pilih.id).delete().then(res=>{
       ...
       refreshData() {
         let that = this
         db.collection(`unit`).onSnapshot(snap=>{
             let arr = []
             snap.forEach(e=>{
    ...
    checkedAll() {
     ...
     susun(datas){
      let that=this
      let data = datas;
      let order = ['id','nama_unit','head_uid']
      let hasil=[]
      let obj={}
