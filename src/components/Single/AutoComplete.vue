...
<template>
  <div>
    <div class="autocomplete" style="z-index:10;">
      <label>
        <span style="position:absolute;z-index:-1;color:white;">test</span>
        <input
          v-model="search"
          type="text"
    ...
    <ul v-show="isOpen" class="autocomplete-results text-center" style="z-index:2000;">
      ...
      >
        {{ result.title }} |
        <span class="font-bold">{{result.keywords}}</span>
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  methods: {
    addData() {
      ...
      axios.get("/api/data/blog/search").then(res => {
        let hasil = res.data.data;
        let hasil2 = res.data.data;
        hasil = hasil.filter(e => e.public == true);
        hasil2 = hasil2.filter(e => e.createdBy == this.$store.state.user._id);
        let diff = _.difference(hasil2, hasil);
        let hasilAkhir = [...hasil, ...diff];
        console.log(hasil);
        console.log(hasil2);
        // hasil = hasil.filter(e=>e.createdBy==this.$store.state.user._id)
        this.items = hasilAkhir;
      });
    },
    enter(e) {
      setTimeout(() => {
    ...
    onEnter() {
      if (this.results[this.arrowCounter] == undefined) {
        ...
        let hasil =
          this.results[this.arrowCounter].kategori + "/" + this.search;
