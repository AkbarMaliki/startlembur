        ...
        <div class="container-fluid" style="padding:10px 40px 40px 20px;">
            <div class="row">
                <div class="col-sm-3">
                    <p class="p-2" style="color:red;font-size:12px;font-family:times new roman;">
                        Note: Barang sedang di proses oleh pelapak
                    </p>
            ...
            <div class="row isiTable " v-for="(item, index) in keranjang" :key="index" style="box-shadow:1px 2px 4px gray;" >
                ...
                <div class="col-sm-2 actionPage ">
                    ...
                    >Batalkan </button>
                    <button type="button" class="btn btn-sm btn-success  btn-block" style="font-size:13px;"
                    @click="kirim(item._id);" v-if="item.idPenjual==$auth.user._id"
                    >Dikirim </button>
                </div>
            </div>
            </template>
...
export default{
    ...
    computed:{
        keranjang(){
           ...
           if(this.$auth.user.email=='admin@gmail.com'){
                ...
                data=data.filter(e=>{
                    return e.status==3  && (e.idPembeli==this.$auth.user._id || e.idPenjual==this.$auth.user._id)
                });
            }
             console.log(data);
            if(data.length>0){
    ...
    methods:{
        ...
        kirim(id){
            ...
            this.$nuxt.$emit('modalId','ResiForm');
            this.$route.query.idTransaksi=id;
        },
    },
    mounted() {
