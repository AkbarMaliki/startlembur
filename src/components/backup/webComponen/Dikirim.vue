        ...
        <div class="container-fluid" style="padding:10px 40px 40px 20px;">
            <div class="row">
                <div class="col-sm-3">
                    <template v-if="keranjang.length>0">
                        <p class="p-2" style="color:red;font-size:12px;font-family:times new roman;"  v-if="keranjang[0].idPembeli==$auth.user._id">
                            Note: Tekan Selesai Apabila barang telah sampai
                        </p>
            ...
            <div class="row isiTable " v-for="(item, index) in keranjang" :key="index" style="box-shadow:1px 2px 4px gray;" >
                ...
                <div class="col-sm-2 actionPage ">
                    ...
                    @click="$nuxt.$emit('modal',true);$nuxt.$emit('modalId','LacakBarang');$route.query.idTransaksi=item.id;$route.query.resi=item.resi;"
                    >Lacak </button>
                    <button type="button" class="btn btn-sm btn-dark  btn-block" style="font-size:13px;"
                    @click="selesai(item._id);$nuxt.$emit('transaksiComponent','Diselesai');" v-if="item.idPembeli==$auth.user._id"
                    >Selesai</button>
                </div>
            </div>
            </template>
...
export default{
    ...
    computed:{
        keranjang(){
            ...
            if(this.$auth.user.email=='admin@gmail.com'){
                ...
                data=data.filter(e=>{
                    return e.status==4  && (e.idPembeli==this.$auth.user._id || e.idPenjual==this.$auth.user._id)
                });
            }
             console.log(data);
            if(data.length>0){
    ...
    methods:{
        ...
        selesai(id){
            this.$axios.put('/api/transaksi/selesai/'+id,{
                headers:{
                    "Authorization":localStorage.getItem('auth._token.local')
                }
            })
