            ...
            <div class="listItem" v-if="barangs.length>0"> 
                <div class="items text-justify wow flipInY   " v-for="(item, index) in barangs" :key="index" >
                     <div class="pull-right">
                        <button class="btn btn-sm btn-outline-danger " style="font-weight:bold;font-size:10px;"
                        @click="deleteBarang(item._id)">
                            Delete
                        </button>
                    </div>
                    <div class="pull-left">
                        <button class="btn btn-sm btn-outline-success " style="font-weight:bold;font-size:10px;"
                        @click="$nuxt.$emit('componentProfile','BarangEditForm');$route.query.barangId=item._id">
                            Edit
                        </button>
                    </div>
                    <br>
  ...
  computed: {
    ...
    barangs() {
      ...
      if (this.$store.state.barang.length > 0) {
        ...
        hasilData = hasilData.filter(
          e => e.createdBy == this.$store.state.auth.user._id
        )
        hasilData = hasilData.filter(e => e.aktif == true)
        // console.log(data);
      } else {
        hasilData = []
      }
