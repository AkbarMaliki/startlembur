    ...
    <template v-if="formOn">
      <div>
        ...
        <div class="container-fluid">
          <div class="row">
            <div class="col-sm-3">
              <p style="font-family:times new roman;font-size:25px;font-weight:bold;">IMPORT DATA :</p>
            </div>
            <div class="col-sm-4">
              <div class="tips">
                <upload @link="linknya" />
                <span class="tipstextB">Klik untuk Import</span>
              </div>
            </div>
            <div class="col-sm-4">
              <div v-if="links">
                ...
                <div class="tips">
                  ...
                  >Simpan</button>
                  <span class="tipstextB">Klik Untuk Simpan</span>
                </div>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <tbody>
              <tr v-for="(item, index3) in td" :key="index3+'td'">
                ...
                <td>
                  <a target="_blank" :href="`https://kantorbamban2.now.sh${item.link}`">
                    <span class="typcn typcn-arrow-maximise"></span> download
                  </a>
                </td>
              </tr>
            </tbody>
...
export default {
  ...
  methods: {
    ...
    saveData() {
      let fd = new FormData();
      fd.append("data[table]", "bamban_tbimport");
      fd.append("data[database]", " infolay3_test");
      fd.append("data[aksi]", "insert");
      fd.append("data[no]", this.vdata.no);

      fd.append("data[nama_file]", this.vdata.file_name);

      fd.append("data[tanggal]", this.formatDate(new Date()));

      fd.append("data[link]", this.links);
